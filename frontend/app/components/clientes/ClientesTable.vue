    ...
    >
      ...
      <template #item.acciones="{ item }">
        <div class="actions-cell">
          ...
          <v-tooltip v-if="isAdmin" text="Inhabilitar cliente">
            <template #activator="{ props }">
              <v-btn
                v-bind="props"
                icon
...
:deep(.clientes-table .v-data-table-header__content) {
  font-weight: 800;
  color: #dfe7f4;
}

:deep(.clientes-table .v-table__wrapper > table > tbody > tr:hover) {
  background: rgba(255, 255, 255, 0.03);
}

:deep(.clientes-table .v-data-table-footer) {
  color: #dfe7f4;
}
